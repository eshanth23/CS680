...
- Each sensor notifies all observers when an event occurs (like speed change, fuel mileage change, or tire pressure change).
- All observers receive and update the same data simultaneously, following the Multicast Design Pattern.

Test Code Successfully Verifies:
1. Whether the event is received by all observers.
2. Whether multiple sensors can send events at the same time.
3. The latest event notification from each sensor.
